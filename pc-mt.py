...
def draw_minimal_pdf(ctx: ToolkitContext) -> None:
    ...
    for page_number, page_lines in enumerate(pages, start=1):
        ...
        footer = f"Generated by {TOOL_NAME} | Session {ctx.session_id} | Page {page_number}"
...
def build_reportlab_pdf(ctx: ToolkitContext) -> None:
    ...
    metadata_rows = [
        ...
        ["Generated By", TOOL_NAME, "Final Status", summary["final_status"]],
    ]
...
def maybe_hold_console_open(args: argparse.Namespace) -> None:
    if args.non_interactive or not sys.stdout.isatty():
        return
    try:
